const envid = 'cloud1-4gweu4m89ff0f881'; //云开发环境id
...
exports.main = async(event, context) => {
      ...
      app.router('recharge', async(ctx) => {
            ...
            //var parse = userinfo.data[0].parse + parseInt(event.num) 
      ...
      app.router('toseller', async(ctx) => {
            ...
            ctx.body = await db.collection('user').doc(userinfo.data[0]._id).update({
                  data: {
                        parse:  Number(userinfo.data[0].parse) + Number(event.num)
                  },
                  success(res){
                        console.log(res);
